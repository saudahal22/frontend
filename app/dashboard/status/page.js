...
export default function StatusPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Opsional: cegah admin masuk halaman user
  ...
  const fetchMyPendaftar = async () => {
    try {
      ...
      // 🛡️ Normalisasi: pastikan kita punya array
      let pendaftarList = [];

      if (Array.isArray(data)) {
        pendaftarList = data;
      } else if (data && Array.isArray(data.data)) {
        // Jika backend kirim { success: true, data: [...] }
        pendaftarList = data.data;
      } else if (data === null || data === undefined) {
        pendaftarList = [];
      } else {
        // Jika backend kirim objek tunggal, jadikan array
        pendaftarList = [data];
      }

      // Ambil pendaftar terbaru
      if (pendaftarList.length > 0) {
        const latest = pendaftarList[0];
        ...
        <div className="container mx-auto px-6 max-w-6xl">
          <FadeIn>
            ...
            <p className="text-lg text-gray-600 text-center max-w-3xl mx-auto mb-16">
              Informasi selanjutnya akan diumumkan dalam 3–5 hari kerja.
            </p>
          </FadeIn>
            ...
            <div className="bg-white/90 backdrop-blur-sm p-8 rounded-3xl shadow-xl border border-white/50 hover:shadow-2xl transition-all duration-300">
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  ...
                  {status === "ditolak" && <span className="text-2xl">💔</span>}
                ...
                <p className="text-sm">
                  {status === "diterima" && (
                    <>
                      Selamat! Anda diterima sebagai anggota Coconut. Instruksi
                      selanjutnya akan disampaikan melalui website.
                    </>
                  )}
                  {status === "ditolak" && (
                    <>
                      Terima kasih atas partisipasi Anda. Kami sangat menghargai
                      usaha Anda. Tetap semangat!
                    </>
                  )}
                  {status === "pending" && (
                    <>
                      Hasil seleksi akan diumumkan dalam waktu 3–5
                      hari kerja. Harap cek website secara berkala.
                    </>
                  )}
                  {status === "Belum Mendaftar" && (
                    <>
                      Anda belum pernah mendaftar. Isi formulir pendaftaran
              ...
              {latestPendaftar && (
                <div className="mt-6 pt-6 border-t border-gray-200 text-sm text-gray-600 space-y-2">
                  <p>
                    <strong>Nama:</strong> {latestPendaftar.nama_lengkap}
                  </p>
                  <p>
                    <strong>Kampus:</strong> {latestPendaftar.asal_kampus}
                  </p>
                  <p>
                    <strong>Program Studi:</strong> {latestPendaftar.prodi}
                  </p>
                  <p>
                    <strong>Semester:</strong> {latestPendaftar.semester}
                  </p>
                  <p>
                    <strong>Tanggal Daftar:</strong>{" "}
                    {new Date(latestPendaftar.created_at).toLocaleDateString(
                      "id-ID"
                    )}
                  </p>
                </div>
